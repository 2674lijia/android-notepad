...
public class SlideCutListView extends ListView {
    ...
    private void scrollLeft() {
        ...
        final int delta = (screenWidth - itemView.getScrollX());
        System.out.println(delta + "aaaaaaaaaaaaaaaaaa");
        // 调用startScroll方法来设置一些滚动的参数，我们在computeScroll()方法中调用scrollTo来滚动item
        scroller.startScroll(itemView.getScrollX(), 0, delta, 0,
                Math.abs(delta));
        postInvalidate(); // 刷新itemView
    ...
    public void computeScroll() {
        ...
        if (scroller.computeScrollOffset()) {
            ...
            if (removeDirection == RemoveDirection.LEFT) {
                itemView.scrollTo(screenWidth / 2, scroller.getCurrY());
                mRemoveListener.leftMoveIten(itemView,slidePosition);

            }else {
                itemView.scrollTo(0,0);
                mRemoveListener.rightMoveIten(itemView,slidePosition);
            }
            postInvalidate();
            // 滚动动画结束的时候调用回调接口
        }
